...
const forgotPassword = async (req, res, next) => {
  ...
  if (!usernameOrEmail || !proposito) {
    return res.status(422).json({
      error: {
        code: "VALIDATION_ERROR",
        message: "Username/email y propósito requeridos",
        timestamp: new Date().toISOString(),
        path: req.path,
      },
    });
  ...
  try {
    ...
    const otpResult = await db.query(
        "SELECT * FROM sp_otp_create($1, $2, $3, $4)",
        [user.user_id, proposito, 300, otpHash],
    );
    let otpId = undefined;
    ...
    res.status(201).json({
      otpId,
      otpCode,
      message: "OTP generado. (En producción se enviaría por email/SMS)",
    });
  } catch (error) {
    next(error);
  }
...
const verifyOtp = async (req, res, next) => {
  ...
  try {
    ...
    const otpDbResult = await db.query(
        "SELECT * FROM Otps WHERE usuario_id = $1 AND proposito = $2 " +
        "AND fecha_consumido IS NULL AND fecha_expiracion > NOW() " +
    ...
    res.status(200).json({
      success: true,
      otpId: otpDb.id,
      message: "OTP verificado y consumido",
    });
  } catch (error) {
    next(error);
  }
